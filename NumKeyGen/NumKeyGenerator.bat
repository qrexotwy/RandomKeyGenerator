@echo off
title Simple KeyGenerator - by @qrexotwy
...
:: Generate keys and save them to the file in numbered format
(for /l %%i in (1,1,%numKeys%) do (
    set key=
    for /l %%j in (1,1,%keyLength%) do (
        set /a rnd=!random! %% 36
        if !rnd! LSS 10 ( 
            set /a rnd+=48
        ) else (
